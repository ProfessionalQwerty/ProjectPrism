...
import { Check, Code2, Copy, Download, ExternalLink } from 'lucide-react'
...
function NpmInstallBlock({ className = '' }: { className?: string }) {
  ...
  return (
    <div className={className}>
      ...
      <p className="mt-1 max-w-md text-[12px] text-neutral-400">{getSmartScreenHint()}</p>
    </div>
  )
}
...
export function LandingPage({ onInstall, onOpenWorkspace }: LandingPageProps) {
  ...
  const npmLabel = getNpmInstallLabel()
      ...
      <nav className="sticky top-0 z-50 border-b border-neutral-200/60 bg-white/70 backdrop-blur-md">
        <div className="mx-auto flex max-w-6xl items-center justify-between px-6 py-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <ButtonColorful label={npmLabel} onClick={() => void copyNpmInstallCommand()} />
          </div>
        </div>
      </nav>
      ...
      <section className="mx-auto max-w-6xl px-6 pb-20 pt-16 text-center">
        ...
        <div className="mt-10 flex flex-col items-center gap-4">
          ...
          <div className="flex flex-wrap items-center justify-center gap-4">
            <ButtonColorful label={npmLabel} onClick={() => void copyNpmInstallCommand()} />
            <Button variant="outline" size="lg" onClick={onInstall} className="border-neutral-300 text-[15px]">
      ...
      <section className="border-t border-neutral-200/80 py-24">
        <div className="mx-auto grid max-w-6xl gap-10 px-6 md:grid-cols-2">
          <div className="rounded-2xl border border-neutral-200/80 bg-white/70 p-8 backdrop-blur-sm">
            ...
            <NpmInstallBlock className="mt-6" />
            <Button variant="outline" size="lg" onClick={onInstall} className="mt-4 border-neutral-300">
              {downloadLabel}
            </Button>
          </div>
      ...
      <section className="border-t border-neutral-200/80 py-20">
        <div className="mx-auto max-w-xl px-6 text-center">
          ...
          <p className="mt-4 text-[16px] text-neutral-600">
            Run the npm command below for Windows, macOS, or Linux. Direct installer download is also available.
          </p>
          <NpmInstallBlock className="mx-auto mt-8" />
          <div className="mt-8 flex flex-wrap justify-center gap-3">
            <ButtonColorful label={npmLabel} onClick={() => void copyNpmInstallCommand()} />
            <Button variant="outline" size="lg" onClick={onInstall}>
              {downloadLabel}
            </Button>
      ...
      <footer className="border-t border-neutral-200/80 py-8">
        <div className="mx-auto flex max-w-6xl flex-col items-center justify-between gap-4 px-6 text-[14px] text-neutral-500 md:flex-row">
          ...
          <div className="flex gap-6">
            ...
            <button type="button" onClick={() => void copyNpmInstallCommand()} className="hover:text-neutral-800">
              Install
            </button>
